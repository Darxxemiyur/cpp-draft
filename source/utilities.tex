...
if the fill option is absent, the space character.
For a format specification in a Unicode encoding,
the fill character corresponds to a single UCS scalar value.
\begin{note}
...
\begin{example}
%FIXME: example is incomplete, sB ans sC result in:
...
\pnum
A (\tcode{0}) option is valid for arithmetic types
...
string s4 = format("{:06}", inf);       // value of \tcode{s4} is "   inf" (\tcode{0} has no effect)
\end{codeblock}
\end{example}
...
the value of the corresponding formatting argument is used as the value
of the option.
...
\pnum
For a sequence of characters in a Unicode encoding,
...
Extended grapheme clusters are defined by \UAX{29}.
...
\pnum
For a sequence of characters in a non-Unicode encoding,
