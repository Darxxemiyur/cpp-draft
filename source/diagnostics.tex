...
%FIXME: We typically use \fmtgrammarterm{} and \fmtnontermdef{} for these,
% but they're not defined until we read utilities.tex.
%FIXME: ... but we can't use \nontermdef and \grammarterm either because
% the build checks flag the terms as being undefined,
\newcommand{\fmtnontermdefTemp}[1]{{\BnfNontermshape#1\itcorr}\textnormal{:}}
\newcommand{\fmtgrammartermTemp}[1]{\gterm{#1}}
%FIXME: ... so the macros above were created as a temporary workaround until
% a proper resolution can be found.
\begin{itemdescr}
\pnum
\tcode{formatter<stacktrace_entry>} interprets \fmtgrammartermTemp{format-spec}
as a \fmtgrammartermTemp{stacktrace-entry-format-spec}.
The syntax of format specifications is as follows:

\begin{ncbnf}
\fmtnontermdefTemp{stacktrace-entry-format-spec}\br
    \opt{fill-and-align} \opt{width}
\end{ncbnf}

\begin{note}
The productions \fmtgrammartermTemp{fill-and-align} and \fmtgrammartermTemp{width}
...
\fmtgrammartermTemp{format-spec} is empty.
